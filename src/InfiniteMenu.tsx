...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const shouldHideText = isMoving || popupItem !== null;
  ...
  setPopupItem(null);
  setShowActionButton(true);   // Show it again
};
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          ...
          <h2 style={{ transform: 'translateX(50px)'}} className={`face-title ${shouldHideText ? 'inactive' : 'active'} ${activeItem.title.length > 20 ? 'long-title' : ''}`}>{activeItem.title}</h2>
          <p style={{ transform: 'translateX(-50px)'}} className={`face-author ${shouldHideText ? 'inactive' : 'active'}`}>{activeItem.author}</p>
          {/* <p className={`face-description ${shouldHideText ? 'inactive' : 'active'}`}>{activeItem.description}</p> */}
          </div>
          <div onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'} ${showActionButton ? 'visible' : 'hidden'}`}>
